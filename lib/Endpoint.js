...
Endpoint.prototype._performOperation = function(method, operationArguments) {
  ...
  var options = {}
  var cb = undefined
  var bodyRequired = ['post', 'put', 'patch'].indexOf(method.toLowerCase()) !== -1
  var index = 0

  // Figure out what the arguments are
  switch (operationArguments.length) {
    case 3:
      body = operationArguments[index++]
    case 2:
      // The case of two args differs per method. In post, put, and patch it is (body, cb).
      options = !_.isNil(body) || !bodyRequired ? operationArguments[index++] : options
      body = _.isNil(body) && bodyRequired ? operationArguments[index++] : body
    case 1:
      cb = operationArguments[index++]
      break
    default:
  }
  ...
  request({
    ...
    if (e) {
      cb(self.httpError(e.code, e.message || e), r)
    } else if (r.statusCode >= 400) {
      cb(self.httpError(r.statusCode, r.body), r)
    } else {
      cb(null, r)
    }
  })
 ...
 *
 * @param params
 * @return a new object with params json values serialized as json strings
 *
 */
