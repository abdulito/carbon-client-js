...
Endpoint.prototype._performOperation = function(method, operationArguments) {
  ...
  request({
    ...
    body: options.json ? EJSON.stringify(body) : body,
    ...
    // parse body
    if(r && r.body && options.json) {
      try {
        r.body = EJSON.parse(r.body)
      } catch(e) {
        // just ignore and leave the body as a plain string
      }
    }
