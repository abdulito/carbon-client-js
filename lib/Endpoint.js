 ...
 * @param {Object} config -- xxx
 * @returns {xxx} -- xxx
 */
Endpoint.prototype.getCollection = function(path, config) {
  var result = undefined

  var endpoint = this.getEndpoint(path)
  if (endpoint) {
    result = new Collection(endpoint, config)
  }
...
Endpoint.prototype._performOperation = function(method, operationArguments) {
  ...
  request({
    ...
    if (e) {
      ...
      var msg = _.get(r, 'body.message') || _.isString(r.body) ? r.body : JSON.stringify(r.body)
  ...
  if (params) {
    for (var key in params) {
      if (_.isArray(params[key])) {
        result[key] = _.map(params[key], function(val) {
          return _.isObject(val) ? ejson.stringify(val) : val
        })
