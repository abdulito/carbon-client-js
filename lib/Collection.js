 ...
 *                skip/limit options of. Default batch size is 100 (Cursor.batchSize).
 *
...
Cursor.prototype.limit = function(val) {
  ...
  this.customLimit = this.batchSize = val
  return this
}
...
Cursor.prototype.getMore = function(cb) {
  ...
  this.collection._doFind(options, function(e, data) {
    if (e) {
      ...
      if (data != null && data.length > 0 && (data.length < self.batchSize)) {
        self.batchSize = data.length
      }

      // handle case where server side collection page size maybe less than client page
      if (self.customLimit != null && (self.customLimit - self.totalFetchedItems < self.batchSize)) {
        self.batchSize = self.customLimit - self.totalFetchedItems
      }
      self.bufferSkip = self.totalFetchedItems
      self.nextItemPos = 0
...
Cursor.prototype._constructFindOptions = function() {
  ...
  _.set(options, "parameters.limit", this.batchSize)
...
Cursor.prototype._recomputeFinished = function() {
  this.finised = !this.isPaginated() || this.items == null || this.items.length == 0 ||
    this.customLimit == this.totalFetchedItems
}
...
function parseFindArgs(findArgs) {
  ...
  // parse custom skip and limit

  if (options && _.get(options, "parameters.skip")) {
    this.skip(_.get(options, "parameters.skip"))
  }

  if (options && _.get(options, "parameters.limit")) {
    this.limit(_.get(options, "parameters.limit"))
  }
