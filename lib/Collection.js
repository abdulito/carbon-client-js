...
function Cursor(collection, findArguments) {
  ...
  this.bufferSkip = 0
  this.customLimit = this.findArguments.options.limit
  this.customSkip = this.findArguments.options.skip
  this.items = null
...
Cursor.prototype.getMore = function(cb, exhaust) {
  ...
  var options = {
    skip: this.customSkip || this.bufferSkip
  }

  if (!exhaust || this.customLimit) {
    options.limit = this.customLimit || this.bufferLimit
  }
...
Cursor.prototype._needToGetMore = function() {
  return this.items == null|| (!this.customLimit &&
