...
CollectionCursor.prototype.each = function(cb) {
  ...
  function callForeachItem(e) {
    if (self.done) {
      ...
      while (self.nextItemPos in self.items) {
        self._nextObject(cb)
      }
...
CollectionCursor.prototype.toArray = function(cb) {
  ...
  function appendItem(e, item) {
    result.push(item)
  }

  // append whats currently available
  while (!this.done && this.items && this.nextItemPos in this.items) {
    this._nextObject(appendItem)
  }
  ...
  function appendRemainingAndCallback(e) {
    ...
    while (!self.done && self.items && self.nextItemPos in self.items) {
      self._nextObject(appendItem)
    }
 ...
 * @private
 */
CollectionCursor.prototype._nextObject = function(cb) {
...
CollectionCursor.prototype._needToGetMore = function() {
  return !this.done && (this.items == null || (this.items.length < this.limit && !this.nextItemPos in this.items))
}
 ...
 * @private
 */
CollectionCursor.prototype._doMoveEach = function(cb) {
  if (this.items) {
    while (this.nextItemPos in this.items) {
      var item = this.items[this.nextItemPos]
      this.nextItemPos++
      cb(null, item)
    }
