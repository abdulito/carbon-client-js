 ...
 *
 *              Abstract interface
 *              insert: function(objects, cb) {},
 *              insertObject: function(obj, cb) {},
...
Collection._reconcileId = function(obj, res) {
  ...
  if (res.statusCode === 201) {
    ...
    // if obj is an array of objects then ensure that _id is an array and matches obj length
    if (_.isArray(obj) && (!_.isArray(_id) || obj.length != _id.length)) {
      throw new Error("Returned _id in headers is not array or does not match length of inserted objects")
    }
 ...
 * @returns {xxx} -- xxx
 */
Collection.prototype.insert = function(objects, cb) {
  return this.endpoint.post(objects, {}, function(err, res) {
    try {
      cb(err, res ? Collection._reconcileId(objects, res) : undefined)
    } catch (e) {
      cb(e)
    }
  })
 ...
 * @returns {xxx} -- xxx
 */
Collection.prototype.save = function(objects, cb) {
  if (!_.isArray(objects)) {
    throw new Error("objects must be an Array")
  }

  return this.endpoint.put(objects, function(err, res) {
    try {
      cb(err, res ? Collection._reconcileId(objects, res) : undefined)
    } catch (e) {
      cb(e)
 ...
 * @returns {object} -- {query: undefined, options: {}, cb: undefined}
 */
