...
Cursor.prototype.limit = function(val) {
  ...
  this.customLimit = this.bufferLimit = val
  return this
}
...
Cursor.prototype.getMore = function(cb) {
  ...
  this.collection._doFind(options, function(e, data) {
    if (e) {
      ...
      if (data != null && data.length > 0 && (data.length < self.bufferLimit)) {
        self.bufferLimit = data.length
      }

      // handle case where server side collection page size maybe less than client page
      if (self.customLimit != null && (self.customLimit - self.totalFetchedItems < self.bufferLimit)) {
        self.bufferLimit = self.customLimit - self.totalFetchedItems
      }
